...
int main(int argc, char *argv[]) {
  ...
  // create a function "foo" that returns int result with two int32 named
  // arguments "a" and "b"
  Function *fooFunc;
  ...
  FunctionType *fooFuncType =
      FunctionType::get(Builder.getInt32Ty(), PTypes, false);
  fooFunc =
      Function::Create(fooFuncType, Function::ExternalLinkage, "foo", ModuleOb);

  std::vector<std::string> Params;
  Params.push_back("a");
  Params.push_back("b");
  int index = 0;
  for (auto it = fooFunc->arg_begin(); it != fooFunc->arg_end();
       ++it, ++index) {
